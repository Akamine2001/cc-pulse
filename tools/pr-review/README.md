...
- ✅ **GitHub Actions Bot（`github-actions[bot]`）からのコメントのみ**
  - 理由1: Jules自身のコメント除外（無限ループ防止）
    - Julesが `@jules` をつけて説明コメントを投稿 → これを送信すると無限ループ
  - 理由2: 人間のコメント除外（二重送信防止）
    - ユーザーが `@jules` をつけてコメント → 既にJulesセッション内で処理されているので二重送信になる
  - 理由3: 自動レビューツールが生成した修正指示のみをJulesに送信
