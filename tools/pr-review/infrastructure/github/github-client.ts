...
export class GitHubClient {
  ...
  /**
   * PRのbase/head SHA情報を取得
   *
   * @param prNumber PR番号
   * @returns base.sha, head.sha
   */
  async getPRInfo(prNumber: number): Promise<{ baseSha: string; headSha: string }> {
    const pr = await this.octokit.rest.pulls.get({
      owner: this.owner,
      repo: this.repo,
      pull_number: prNumber
   ...
   * @param path ファイルパス
   * @param line 行番号
   * @param side 'LEFT' (削除行) または 'RIGHT' (追加/変更行)
   * @param body コメント本文
   */
