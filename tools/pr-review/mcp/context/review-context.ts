...
import type { GuidelinesFile, Guideline } from '../../shared/guidelines-types';
...
export class ReviewContext {
  ...
  getUncheckedGuideline(): Guideline | null {
    if (!this.guidelinesData) {
        return null;
    }
    const unchecked = this.guidelinesData.guidelines.find(g => !g.checked);
