...
**観点を出力する前に、矛盾がないか確認し、必要に応じて修正してください。**

#### 6-1. 矛盾パターンの検出

以下のパターンがないか確認してください：

**パターンA: 「変更禁止」と「追加必須」の衝突**

同じ対象（ファイル/関数）に対して、以下の両方が存在する場合は矛盾の可能性があります：
- 「そのまま移植」「ロジック変更なし」「既存と同じ」
- 「〜を追加」「〜を実装」「〜が表示されるか」

**確認手順**:
1. 「変更禁止」系の観点の対象ファイル/関数を特定
2. 「追加必須」系の観点の対象ファイル/関数を特定
3. 対象が重複している場合、既存コードに該当機能があるか確認
4. 既存に機能がなければ → **矛盾確定**

**パターンB: 親Issueの完了条件との不整合**

- 親Issueの「完了条件」で「Xを実装」と明記されている
- 観点で「既存のまま」「変更なし」と記載している
- 既存コードにXがない
...
矛盾を検出した場合、以下のルールで解決してください：

**ルール1: 親Issueの「完了条件」を最優先**

完了条件に明記されている要件は必ず満たす必要があります。
それと矛盾する観点は修正してください。

**ルール2: 「既存パターン」は参考情報として扱う**

「既存と同じ」という観点は、完了条件を満たすための変更を許容する形に修正してください。

**修正例**:

❌ **修正前（矛盾あり）**:
```
- 観点A: 既存の関数をそのまま移植しているか（ロジック変更なし）
- 観点B: 各操作に -h オプションでヘルプが表示されるか
```
（既存に-hがない場合、両立しない）

✅ **修正後（矛盾解消）**:
```
- 観点A: 既存の関数のコアロジックを維持しているか（-hオプション追加は親Issueの完了条件のため許容）
- 観点B: 各操作に -h オプションでヘルプが表示されるか
```

**ルール3: 形式の不整合は明確化**

「既存のフォーマット（日本語）と統一」と「新機能にもヘルプを追加」が両方ある場合、
新機能のヘルプも日本語で書くべきか明確にしてください。

#### 6-3. 再確認

修正後、再度矛盾がないか確認してください。
矛盾がなければ `create_review_guidelines` で出力してください。
