...
install_gh() {
    ...
    if command -v gh &> /dev/null; then
        ...
        log_info "To upgrade manually, remove the existing installation and run this script again"
        return
    fi
    ...
    TMP_DIR=$(mktemp -d)
    TMP_FILE="$TMP_DIR/gh.tar.gz"

    log_info "Downloading GitHub CLI from $GH_URL..."
    if ! curl -L -o "$TMP_FILE" "$GH_URL"; then
        rm -rf "$TMP_DIR"
    ...
    if ! sudo cp "$TMP_DIR/gh_${GH_VERSION}_linux_${GH_ARCH}/bin/gh" /usr/local/bin/gh; then
        rm -rf "$TMP_DIR"
        error_exit "Failed to copy binary to /usr/local/bin"
    fi

    if ! sudo chmod +x /usr/local/bin/gh; then
        rm -rf "$TMP_DIR"
